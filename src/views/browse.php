              ...
              <div id='product-list' class='col l-10 c-12'>
                <div class='row no-gutter'>
                  <?php
                    ...
                    $limit = 8;
                    $url = createUrl("src/views/browse.php?page=$current_page");
                    $paging = paging($url, $total_records, $current_page, $limit);
                    $sql ="SELECT * FROM product LIMIT {$paging['start']}, {$paging['limit']}";
                    $result = $db->query($sql);
                ...
                <div id="page-list" class="row flex-center">
                  <?=$paging['html'];?>
                </div>
              </div>
            </div>
