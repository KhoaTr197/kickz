$(document).ready(function () {
  ...
  $(".filter-list__item input").click(function (e) {
    ...
    if (url.searchParams.has(queryStr[0])) {
      ...
      if ($(this)[0].checked) url.searchParams.append(queryStr[0], queryStr[1]);
    } else {
  ...
  $(".table-action__disable-btn").click(function (event) {
    ...
    if (
      confirm(
        "Bạn có chắc chắn muốn tiếp tục không? (Đối với Sản Phẩm, Người Dùng sẽ chỉ vô hiệu hóa)"
      )
    ) {
  ...
  $(".table-action--receipt a")
    .not(".table-action__detail-btn")
